...
//loading the core tensorflow.js library
const tf=require('@tensorflow/tfjs');
//normalize the value between the given range
function normalize(value, min, max) {
    if (min === undefined || max === undefined) {
      return value;
    }
/*undefined is used in a case where no value
* has been assigned to the variable, JavaScript calls it ‘undefined’.*/
...
/*columnConfig is a congiguration model only used when creating a columnmodel
* ColumnModel is used to configure how and what parts
* of that data will be displayed in the grid*/
    const trainingData =
...
//need to definr the shape of the first input layer
model.add(tf.layers.dense({units: 250, activation: 'relu', inputShape: [8]}));
model.add(tf.layers.dense({units: 175, activation: 'relu'}));
model.add(tf.layers.dense({units: 150, activation: 'relu'}));
model.add(tf.layers.dense({units: NUM_PITCH_CLASSES, activation: 'softmax'}));
//we can use sparseCategoricalCrossentropy when our classes are mutually exclusive
model.compile({
  optimizer: tf.train.adam(),
  loss: 'sparseCategoricalCrossentropy',
  metrics: ['accuracy']
});
//predicting a pitch type for single module
